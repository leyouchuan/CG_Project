...
void main() {
    ...
    if (isSnow) {
        ...
        if (abs(pos.y + 2.1) < 0.2) {
            ...
            if (footprint.a > 0.01) {
                ...
                // roughness还可以影响表面细节
                float detail = snowSurfaceDetail(pos, normal);
                float detailAmount = mix(0.0, 0.3, footprintRoughness);
                color = mix(color, footprintColor, colorMix);
                color *= (1.0 - detail * detailAmount);  // 粗糙度越高，细节越多
            }
        }
        ...
        // 环境光
        vec3 ambient = ambientStrength * objectColor * occlusion;
    ...
    FragColor.rgb += footprintColor * 0.0;
    FragColor.rgb *= (1.0 + materialShininess * 0.0);
}
